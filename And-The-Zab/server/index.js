...
const maxUploadSize = isVercel ? 4 * 1024 * 1024 : 25 * 1024 * 1024
const maxImageSize = isVercel ? 4 * 1024 * 1024 : 5 * 1024 * 1024
const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: maxUploadSize }, fileFilter: (req, file, done) => done(null, /^audio\/(mpeg|wav|ogg|mp4)$/.test(file.mimetype)) })
...
app.post('/api/blob/upload', requireUser, async (req, res, next) => { try { if (!process.env.BLOB_READ_WRITE_TOKEN) return res.status(503).json({ error: 'Direct uploads are not configured.' }); const jsonResponse = await handleUpload({ request: req, body: req.body, onBeforeGenerateToken: async () => ({ allowedContentTypes: ['audio/mpeg', 'audio/wav', 'audio/ogg', 'audio/mp4'], maximumSizeInBytes: 25 * 1024 * 1024, addRandomSuffix: true, tokenPayload: JSON.stringify({ userId: String(req.userId) }) }), onUploadCompleted: async () => {} }); res.json(jsonResponse) } catch (error) { next(error) } })
app.post('/api/album/upload-complete', requireUser, async (req, res, next) => { try { const audioUrl = String(req.body?.audioUrl || '').trim(), title = String(req.body?.title || '').trim(); if (!audioUrl || !title || !audioUrl.startsWith('https://')) return res.status(400).json({ error: 'Uploaded audio information is incomplete.' }); const item = { userId: String(req.userId), videoId: `upload-${crypto.randomUUID()}`, title, artist: 'My music', audioUrl, duration: '', thumbnail: '', createdAt: new Date() }; if (useMemoryStore()) { memoryStore.albumItems.push(item); return res.status(201).json({ id: item.videoId, title: item.title, artist: item.artist, duration: '', thumbnail: '', audioUrl: item.audioUrl }) } const saved = await AlbumItem.create(item); res.status(201).json({ id: saved.videoId, title: saved.title, artist: saved.artist, duration: '', thumbnail: '', audioUrl: saved.audioUrl }) } catch (error) { next(error) } })
app.post('/api/album/upload', requireUser, upload.single('audio'), async (req, res, next) => { try { if (!req.file) return res.status(400).json({ error: `Choose an MP3, WAV, OGG, or M4A audio file (maximum ${Math.floor(maxUploadSize / 1024 / 1024)} MB).` }); const originalName = decodeUploadFilename(req.file.originalname); const title = String(req.body.title || path.parse(originalName).name).trim(); const artist = String(req.body.artist || 'My music').trim(); const audioUrl = await storeUpload(req.file, 'audio'); if (useMemoryStore()) { const item = { userId: String(req.userId), videoId: `upload-${crypto.randomUUID()}`, title, artist, audioUrl, duration: '', thumbnail: '', createdAt: new Date() }; memoryStore.albumItems.push(item); return res.status(201).json({ id: item.videoId, title: item.title, artist: item.artist, duration: item.duration, thumbnail: item.thumbnail, audioUrl: item.audioUrl }) } const item = await AlbumItem.create({ userId: req.userId, videoId: `upload-${crypto.randomUUID()}`, title, artist, audioUrl }); res.status(201).json({ id: item.videoId, title: item.title, artist: item.artist, duration: '', thumbnail: '', audioUrl: item.audioUrl }) } catch (error) { next(error) } })
...
app.use((error, req, res, next) => { console.error(error); if (error?.code === 'LIMIT_FILE_SIZE') return res.status(413).json({ error: `File is too large. The maximum size is ${Math.floor(maxUploadSize / 1024 / 1024)} MB on this deployment.` }); res.status(500).json({ error: 'Something went wrong. Please try again.' }) })
